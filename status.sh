...
ssh -i "$SSH_KEY_PATH" "$REMOTE_USER@$REMOTE_HOST" "cd $REMOTE_DIR && docker-compose ps"

echo ""
echo "=== Recent Logs (last 30 lines) ==="
ssh -i "$SSH_KEY_PATH" "$REMOTE_USER@$REMOTE_HOST" "cd $REMOTE_DIR && docker-compose logs --tail 30"
