...
impl Config {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        match self.strategy.alpha_source.as_str() {
            "binance" | "standx" => {}
            other => {
                return Err(ConfigError::ValidationError(format!(
                    "strategy.alpha_source must be \"binance\" or \"standx\", got \"{}\"",
                    other
                )));
            }
        }

        if self.strategy.alpha_source == "binance" && self.strategy.binance_stale_ms == 0 {
            return Err(ConfigError::ValidationError(
                "strategy.binance_stale_ms must be greater than 0 when alpha_source=\"binance\""
                    .to_string()
            ));
        }

        if self.strategy.lot_size <= 0.0 || !self.strategy.lot_size.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.lot_size must be positive and finite".to_string()
            ));
        }

        if !self.strategy.tick_size.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.tick_size must be finite".to_string()
            ));
        }

        if self.strategy.half_spread < 0.0 || !self.strategy.half_spread.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.half_spread must be non-negative and finite".to_string()
            ));
        }

        if self.strategy.half_spread_bps < 0.0 || !self.strategy.half_spread_bps.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.half_spread_bps must be non-negative and finite".to_string()
            ));
        }

        if self.strategy.min_half_spread_bps < 0.0 || !self.strategy.min_half_spread_bps.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.min_half_spread_bps must be non-negative and finite".to_string()
            ));
        }

        if !self.strategy.c1.is_finite() || !self.strategy.c1_ticks.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.c1 and strategy.c1_ticks must be finite".to_string()
            ));
        }

        if !self.strategy.vol_to_half_spread.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.vol_to_half_spread must be finite".to_string()
            ));
        }

        if !self.strategy.skew.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.skew must be finite".to_string()
            ));
        }

        if !self.strategy.spread_level_multiplier.is_finite() {
            return Err(ConfigError::ValidationError(
                "strategy.spread_level_multiplier must be finite".to_string()
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_invalid_alpha_source() {
        // Typo in alpha_source must be rejected, not silently fall back to StandX.
        let mut config = Config::default();
        config.strategy.alpha_source = "binanace".to_string();
        assert!(config.validate().is_err(), "typo in alpha_source must be rejected");

        config.strategy.alpha_source = "".to_string();
        assert!(config.validate().is_err(), "empty alpha_source must be rejected");

        config.strategy.alpha_source = "binance".to_string();
        assert!(config.validate().is_ok(), "binance is valid");

        config.strategy.alpha_source = "standx".to_string();
        assert!(config.validate().is_ok(), "standx is valid");
    }

    #[test]
    fn test_zero_step_ns_rejected() {
        let mut config = Config::default();
    ...
    #[test]
    fn test_zero_update_interval_steps_rejected() {
        let mut config = Config::default();
    ...
    fn test_nonfinite_floats_rejected() {
        let mut config = Config::default();
        config.strategy.half_spread = f64::NAN;
        assert!(config.validate().is_err(), "NaN half_spread must be rejected");

        config = Config::default();
        config.strategy.half_spread_bps = f64::INFINITY;
        assert!(config.validate().is_err(), "Infinity half_spread_bps must be rejected");

        config = Config::default();
        config.strategy.skew = f64::NAN;
        assert!(config.validate().is_err(), "NaN skew must be rejected");

        config = Config::default();
        config.strategy.c1 = f64::INFINITY;
        assert!(config.validate().is_err(), "Infinity c1 must be rejected");
    }

    #[test]
    fn test_negative_half_spread_bps_rejected() {
        let mut config = Config::default();
        config.strategy.half_spread_bps = -1.0;
        assert!(config.validate().is_err());
    }
